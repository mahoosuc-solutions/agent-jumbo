#!/usr/bin/env bash
# ============================================
# Agent Mahoo — Deploy Host: Resolve Runtime Env
# ============================================
...
# and writes a runtime .env file for Agent Mahoo.
#
...
#   ENV_OUTPUT       Output file path (default: /opt/agent-mahoo/runtime.env)
#
...
ENV_OUTPUT="${ENV_OUTPUT:-/opt/agent-mahoo/runtime.env}"
...
echo "[resolve-env] Agent Mahoo runtime env resolution"
...
# ── Resolve Agent Mahoo runtime secrets ───────────────────────────────────────
# Path convention: secret/data/platform/system/agent-mahoo/<secret_name>
AGENT_MAHOO_PATH="platform/system/agent-mahoo/runtime"
echo "[resolve-env] Reading: ${VAULT_MOUNT}/data/${AGENT_MAHOO_PATH}"

RUNTIME_RESP=$(vault_read "${AGENT_MAHOO_PATH}" 2>/dev/null || true)

if [[ -z "${RUNTIME_RESP}" ]]; then
    echo "[resolve-env] ERROR: could not read ${AGENT_MAHOO_PATH} from Vault" >&2
...
if [[ ${#MISSING[@]} -gt 0 ]]; then
    echo "[resolve-env] ERROR: required secrets missing from Vault path ${AGENT_MAHOO_PATH}:" >&2
    for m in "${MISSING[@]}"; do
        echo "[resolve-env]   - ${m}" >&2
    done
    exit 1
...
# Agent Mahoo Runtime Configuration
