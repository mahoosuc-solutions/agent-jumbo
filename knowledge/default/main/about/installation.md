...
The following user guide provides instructions for installing and running Agent Mahoo using Docker, which is the primary runtime environment for the framework. For developers and contributors, we also provide instructions for setting up the [full development environment](#in-depth-guide-for-full-binaries-installation).
...
- This directory will contain all your Agent Mahoo files, like the legacy root folder structure:
  ...
  - `.env` - Your API keys
  - `settings.json` - Your Agent Mahoo settings
...
- Under "Volumes", configure:
  - Host path: Your chosen directory (e.g., `C:\agent-mahoo-data`)
  - Container path: `/a0`
...
>
> ```bash
> docker run -p $PORT:80 -v /path/to/your/data:/aj agent0ai/agent-mahoo
> ```
>
...
Learn more about Remote Function Calls and their purpose [here](#7-configure-agent-mahoo-rfc).
...
> mixed in the vector database. Note that this will DELETE ALL of Agent Mahoo's memory.
...
For developers or users who need to run Agent Mahoo directly on their system,see the [In-Depth Guide for Full Binaries Installation](#in-depth-guide-for-full-binaries-installation).
...
- Using Git/GitHub: Pull the latest version of the Agent Mahoo repository.
...
- Agent Mahoo is a framework. It's made to be customized, edited, enhanced. Therefore you need to install the necessary components to run it when downloading its full binaries. This guide will help you to do so.
- The following step by step instructions can be followed along with a video for this tutorial on how to make Agent Mahoo work with its full development environment.
...
2. You don't necessarily need API keys: Agent Mahoo can run with local models. For this tutorial though, we will leave it to the default OpenAI API. A guide for downloading Ollama along with local models is available [here](#installing-and-using-ollama-local-models).
...
6. Running Agent Mahoo without Docker makes the process more complicated and it's thought for developers and contributors.
...
## 2. Download Agent Mahoo

- You can clone the Agent Mahoo repository (<https://github.com/agent0ai/agent-mahoo>) from GitHub if you know how to use Git. In this tutorial I will just show how to download the files.

1. Go to the Agent Mahoo releases [here](https://github.com/agent0ai/agent-mahoo/releases).
...
3. Extract the downloaded archive where you want to have it. I will extract them to "agent-mahoo" folder on my Desktop - "C:\Users\frdel\Desktop\agent-mahoo" on Windows and "/Users/frdel/Desktop/agent-mahoo" on macOS.
...
2. In the terminal, navigate to your Agent Mahoo folder using **"cd"** command. Replace the path with your actual Agent Mahoo folder path.

~~~
cd C:\Users\frdel\Desktop\agent-mahoo
~~~
...
3. Create Conda environment using command **"conda create"**. After **"-n"** is your environment name, you can choose your own, i will use **"a0"** - short for Agent Mahoo. After **"python"** is the Python version that Conda will install for you into this environment, right now, 3.12 works fine. **-y** skips confirmations.
...
Simply put, Docker is a way of running virtual computers on your machine. These are lightweight, disposable and isolated from your operating system, so it is a way to sandbox Agent Mahoo.

- Agent Mahoo only connects to the Docker container when it needs to execute code and commands. The frameworks itself runs on your machine.
...
> Docker socket to be used (requires password)."  This allows Agent Mahoo to
...
- Agent Mahoo needs a Docker image to be pulled from the Docker Hub to be run, even when using the full binaries.
You can refer to the [installation instructions above](#windows-macos-and-linux-setup-guide) to run the Docker container and then resume from the next step. There are two differences:
  ...
  - You need to map the `/a0` volume to the location of your local Agent Mahoo folder.
- Run the Docker container following the instructions.

## 5. Run the local Agent Mahoo instance

Run the Agent Mahoo with Web UI:
...
- Open the URL shown in terminal in your web browser. You should see the Agent Mahoo interface.

## 6. Configure Agent Mahoo

Now we can configure Agent Mahoo - select models, settings, API Keys etc. Refer to the [Usage](usage.md#agent-configuration) guide for a full guide on how to configure Agent Mahoo.

## 7. Configure Agent Mahoo RFC

Agent Mahoo needs to be configured further to redirect some functions to the Docker container. This is crucial for development as A0 needs to run in a standardized environment to support all features.
...
9. Now you have the full development environment to work on Agent Mahoo.
