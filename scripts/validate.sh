#!/bin/bash
# Quick validation script to check Agent Mahoo deployment

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

echo "🔍 Agent Mahoo Deployment Validation"
echo "===================================="
echo ""

# Check Docker containers
echo "📦 Container Status:"
docker ps --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}" | grep -E "(agent-mahoo|agent-mahoo|ollama)" || echo "⚠️  Containers not running"
echo ""

# Check UI accessibility
echo "🌐 Web Interface:"
if curl -s http://localhost:50080 > /dev/null; then
    echo "✓ Agent Mahoo UI accessible at http://localhost:50080"
else
    echo "✗ Agent Mahoo UI not accessible"
fi
echo ""
...
if [ -d "${ROOT_DIR}/docker/run/agent-mahoo/data" ]; then
    echo "Database directory exists"
    ls -lh "${ROOT_DIR}"/docker/run/agent-mahoo/data/*.db 2>/dev/null | awk '{print "  - " $9 " (" $5 ")"}' || echo "  No databases created yet (will be created on first use)"
else
...
echo "✓ Agent Mahoo UI: http://localhost:50080"
