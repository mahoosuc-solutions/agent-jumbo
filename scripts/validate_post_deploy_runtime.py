...
def manual_smoke_markdown(
    *,
    ...
    browser = artifacts.get("browser", {})
    scheduler_api = artifacts.get("scheduler_api", {})
    scheduler_after_restart = artifacts.get("scheduler_api_after_restart", {})

    def result_line(name: str) -> str:
        match = next((result for result in results if result.name == name), None)
        if not match:
            return "not run"
        return "passed" if match.ok else f"failed ({match.detail})"

    lines = [
        f"# Manual Smoke Record — {timestamp[:8]}",
        "",
        f"- Environment: production Docker target `{container}`",
        f"- Base URL: `{base_url}`",
        f"- Browser URL: `{browser_base_url}`",
        "- Tester: Codex",
        "- Timezone: `America/New_York`",
        "",
        "## Scope",
        "",
        "This smoke record covers the current GA runtime, dashboard routing, trust UX, and persisted scheduler state.",
        "",
        "## Runtime Checks",
        "",
        "1. Health and readiness proxy",
        f"   Result: {result_line('health')}",
        f"   Evidence: `{next((r.detail for r in results if r.name == 'health'), '')}`",
        "",
        "2. Scheduler persistence",
        f"   Result: {result_line('scheduler_file')}",
        f"   Evidence: `tasks={scheduler_api.get('task_count', 0)}` before restart",
    ]
    if restart_container:
        lines.extend(
            [
                f"   Restart result: {result_line('restart')}",
                f"   Evidence after restart: `tasks={scheduler_after_restart.get('task_count', 0)}`",
            ]
        )
    lines.extend(
        [
            "",
            "3. Work Queue, Workflows, Tasks, Trust & Security, New Chat",
            f"   Result: {result_line('browser_smoke')}",
            "   Evidence:",
            f"   - Work Queue heading: `{browser.get('work_queue', {}).get('heading', '')}`",
            f"   - Workflows active dashboard: `{browser.get('workflows', {}).get('active_dashboard', '')}`",
            f"   - Tasks cards: `{browser.get('tasks', {}).get('cards', 0)}`",
            f"   - Trust level cards: `{browser.get('trust', {}).get('level_cards', 0)}`",
            f"   - New chat changed: `{browser.get('new_chat', {}).get('changed', False)}`",
            "",
            "## Notes",
            "",
            "- This record is generated by `scripts/validate_post_deploy_runtime.py`.",
            "- Re-run within the final 72-hour launch window for the GA decision packet.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def write_artifacts(
    *,
    ...
    markdown_path = directory / f"manual-smoke-{timestamp}.md"
    json_path = directory / f"runtime-validation-{timestamp}.json"
...
def main() -> int:
    ...
    parser.add_argument(
        "--write-artifacts",
        action="store_true",
        help="Write manual smoke markdown and JSON artifacts under --report-dir.",
    )
    parser.add_argument(
        "--restart-container",
        action="store_true",
    ...
    if args.write_artifacts:
        ...
        print(f"[INFO] manual_smoke_artifact: {markdown_path}")
        print(f"[INFO] runtime_validation_json: {json_path}")
