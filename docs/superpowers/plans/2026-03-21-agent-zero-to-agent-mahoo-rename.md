...
**Goal:** Rename all references from agent-mahoo/Agent Mahoo/a0 to agent-mahoo/Agent Mahoo/aj across the entire codebase, completing the clean fork break from upstream.
...
| Old | New | Context |
|-----|-----|---------|
| `agent-mahoo` | `agent-mahoo` | Kebab-case (Docker, URLs, filenames) |
| `agent_mahoo` | `agent_mahoo` | Snake_case (Python, env vars) |
| `Agent Mahoo` | `Agent Mahoo` | Display name (UI, docs) |
| `AgentZero` | `AgentMahoo` | PascalCase (if any) |
| `agent0` | `agent-mahoo` | Agent directory name |
| `/aj/` `/a0` | `/aj/` `/aj` | Container internal path |
| `.a0proj` | `.ajproj` | Project metadata directory |
| `agent_mahoo_data` | `agent_mahoo_data` | Docker volume name |
| `agent-mahoo-local` | `agent-mahoo:latest` | Docker image name |
| `agent0ai/agent-mahoo-base` | kept as-is | Base image (external dependency — rename later if we build our own base) |

**IMPORTANT EXCLUSIONS:**

- `docker/run/agent-mahoo/` — Will be REMOVED entirely (upstream copy)
...
- Remove: `docker/run/agent-mahoo/` (entire directory)
...
- [ ] **Step 2: Remove the upstream agent-mahoo directory**

```bash
rm -rf docker/run/agent-mahoo/
```

Note: This directory is root-owned from Docker. May need `sudo rm -rf docker/run/agent-mahoo/`.

- [ ] **Step 3: Commit**

```bash
git add -A docker/run/agent-mahoo/
git commit -m "chore: remove upstream agent-mahoo copy — clean fork break"
```
...
- Service name: `agent-mahoo` → `agent-mahoo`
- Container name: `agent-mahoo` → `agent-mahoo`
- Image: `agent-mahoo-local:latest` → `agent-mahoo:latest`
- Volume: `./agent-mahoo:/a0` → `./agent-mahoo:/aj`
- Volume name: `agent_mahoo_data` → `agent_mahoo_data`
- Named volume definition: `agent_mahoo_data` → `agent_mahoo_data`
...
- `install_aj.sh`: all `/a0` paths → `/aj`, any "agent-mahoo" references → "agent-mahoo"
- `install_aj2.sh`: same
- `copy_aj.sh`: `/a0` → `/aj`
...
- `docker/build-fast.sh`: `agent-mahoo` → `agent-mahoo`, image names
- `docker/build-base-local.sh`: `agent-mahoo` → `agent-mahoo`
- `docker/base/build.txt`: `agent-mahoo` → `agent-mahoo`
- `docker/run/build.txt`: `agent-mahoo` → `agent-mahoo`

- [ ] **Step 5: Commit**

```bash
git add docker/
git commit -m "feat(docker): rename agent-mahoo → agent-mahoo across all Docker infrastructure"
```
...
### Task 4: Python Code — agent-mahoo/agent_mahoo Name References
...
find . -maxdepth 1 -name "*.py" -exec sed -i 's/agent-mahoo/agent-mahoo/g; s/agent_mahoo/agent_mahoo/g; s/Agent Mahoo/Agent Mahoo/g; s/AgentZero/AgentMahoo/g' {} +
find python/ instruments/ tests/ -name "*.py" -exec sed -i 's/agent-mahoo/agent-mahoo/g; s/agent_mahoo/agent_mahoo/g; s/Agent Mahoo/Agent Mahoo/g; s/AgentZero/AgentMahoo/g' {} +
```

- [ ] **Step 2: Special case — update_check.py GitHub URL**

Verify `python/helpers/update_check.py` — the GitHub repo URL `agent0ai/agent-mahoo` should be updated to point to your fork or disabled entirely since we're no longer tracking upstream.
...
git commit -m "refactor: rename agent-mahoo → agent-mahoo in all Python code"
```
...
```bash
find webui/ -name "*.js" -o -name "*.html" | xargs sed -i 's/agent-mahoo/agent-mahoo/g; s/agent_mahoo/agent_mahoo/g; s/Agent Mahoo/Agent Mahoo/g; s|/aj/|/aj/|g; s|/a0"|/aj"|g'

# JSON files
find . -maxdepth 1 -name "*.json" -exec sed -i 's/agent-mahoo/agent-mahoo/g; s/agent_mahoo/agent_mahoo/g; s/Agent Mahoo/Agent Mahoo/g; s|/aj/|/aj/|g' {} +
find .devcontainer/ app_spec/ memory/ instruments/ -name "*.json" -exec sed -i 's/agent-mahoo/agent-mahoo/g; s/agent_mahoo/agent_mahoo/g; s/Agent Mahoo/Agent Mahoo/g; s|/aj/|/aj/|g' {} +
```
...
git commit -m "feat(ui): rebrand Agent Mahoo → Agent Mahoo in all UI and JSON files"
```
...
```bash
git mv agents/agent0 agents/agent-mahoo
```

- [ ] **Step 2: Bulk sed replacement in prompts, knowledge, agents, conf**

```bash
find prompts/ agents/ knowledge/ conf/ -type f \( -name "*.md" -o -name "*.yaml" -o -name "*.yml" \) -exec sed -i 's/agent-mahoo/agent-mahoo/g; s/agent_mahoo/agent_mahoo/g; s/Agent Mahoo/Agent Mahoo/g; s/agent0/agent-mahoo/g; s|/aj/|/aj/|g; s|/a0 |/aj |g' {} +
```

- [ ] **Step 3: Update initialize.py to reference new agent directory**

Check if `initialize.py` or any config references `agents/agent0` as the default agent and update to `agents/agent-mahoo`.
...
git commit -m "refactor: rebrand prompts, agents, knowledge to Agent Mahoo"
```
...
- `docs/LEGAL_COMPLIANCE_BRIEF_AGENT_MAHOO.md` (3 refs)
- `docs/COMMUNICATION_PACK_AGENT_MAHOO.md` (3 refs)
- `docs/GO_BLUE_EXECUTION_PLAN_AGENT_MAHOO.md` (2 refs)
...
- `.env` (check for any agent-mahoo refs)

- [ ] **Step 1: Bulk sed replacement in scripts**

```bash
find scripts/ -name "*.sh" -exec sed -i 's/agent-mahoo/agent-mahoo/g; s/agent_mahoo/agent_mahoo/g; s/Agent Mahoo/Agent Mahoo/g; s|/aj/|/aj/|g; s|/a0"|/aj"|g' {} +
sed -i 's/agent-mahoo/agent-mahoo/g; s/agent_mahoo/agent_mahoo/g; s|/aj/|/aj/|g' init_mcp.sh
```

- [ ] **Step 2: Bulk sed replacement in docs**

```bash
find docs/ -name "*.md" -exec sed -i 's/agent-mahoo/agent-mahoo/g; s/agent_mahoo/agent_mahoo/g; s/Agent Mahoo/Agent Mahoo/g; s|/aj/|/aj/|g' {} +
```
...
for f in DEV_CONTAINER_FIX.md DEV_CONTAINER_READY.md .github/copilot-instructions.md; do
  sed -i 's/agent-mahoo/agent-mahoo/g; s/agent_mahoo/agent_mahoo/g; s/Agent Mahoo/Agent Mahoo/g; s|/aj/|/aj/|g' "$f"
done
```
...
```bash
git mv docs/AGENT_ZERO_PRODUCT_UPDATE_DRAFT.md docs/AGENT_MAHOO_PRODUCT_UPDATE_DRAFT.md
```

- [ ] **Step 5: Update .claude/settings.local.json**

```bash
sed -i 's/agent-mahoo/agent-mahoo/g; s/agent_mahoo/agent_mahoo/g; s|/aj/|/aj/|g; s|/a0"|/aj"|g' .claude/settings.local.json
```
...
git commit -m "refactor: rename agent-mahoo → agent-mahoo in scripts, docs, and configs"
```
...
```bash
find instruments/ -name "*.md" -exec sed -i 's/agent-mahoo/agent-mahoo/g; s/agent_mahoo/agent_mahoo/g; s/Agent Mahoo/Agent Mahoo/g; s|/aj/|/aj/|g' {} +
```
...
```bash
git add instruments/
git commit -m "refactor: rename agent-mahoo refs in instrument documentation"
```
...
- [ ] **Step 1: Verify no remaining agent-mahoo references in project code**

```bash
grep -rl "agent.zero\|agent-mahoo\|agent_mahoo\|Agent Mahoo\|AgentZero" --include="*.py" --include="*.js" --include="*.html" --include="*.json" --include="*.yml" --include="*.yaml" --include="*.md" --include="*.sh" --include="*.cfg" --include="*.conf" --include="*.toml" . | grep -v ".git/" | grep -v "logs/" | grep -v "__pycache__/" | grep -v "node_modules/" | grep -v "vendor/"
```
...
1. Remove upstream agent-mahoo copy
2. Docker infrastructure rename
3. Python /aj/ path references
4. Python agent-mahoo name references
...
```bash
cd docker/run
# Create the agent-mahoo directory (replaces old agent-mahoo mount)
# This should be a fresh checkout or copy of the project
cp -r /mnt/wdblack/dev/projects/agent-mahoo docker/run/agent-mahoo

# Rebuild with new image name
docker build --build-arg BRANCH=main -t agent-mahoo:latest .
...
- The `agent0ai/agent-mahoo-base:latest` base Docker image reference in `Dockerfile` is kept as-is for now — it's an external dependency. Building our own base image is a separate task.
- Old HTML log files in `logs/` are left as-is — they're historical artifacts.
- The `memory/embeddings/` directories contain hash-based names that happen to include "a0" substrings in SHA hashes — these are NOT agent-mahoo references and should not be touched.
