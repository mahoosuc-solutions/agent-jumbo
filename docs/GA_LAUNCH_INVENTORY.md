...
| Payment dunning and retry | internal | Engineering | `payment_dunning` tool `run_cycle` action against test past_due invoices | dunning cycle log | disable `DunningManager` cron call | Internal — no customer-visible surface; validate before exposing |
| Browser-assisted payment account setup | internal | Engineering | `payment_account_setup` tool `start_setup` dry-run with mock browser | setup session log | tool is agent-only, no customer surface | Internal — browser automation instrument; validate Playwright MCP before operator use |
