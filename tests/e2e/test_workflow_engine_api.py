...
def test_workflow_list(app_server, auth_cookies):
    ...
    data = api_post(
        app_server, auth_cookies, "workflow_engine_api", {"action": "list_workflows"}
    )
    ...
    assert isinstance(data["workflows"], list), (
        f"Expected 'workflows' to be a list, got: {type(data['workflows'])}"
    )


@pytest.mark.e2e
def test_workflow_dashboard_engine(app_server, auth_cookies):
    """POST workflow_engine_api with action get_dashboard returns success."""
    data = api_post(
        app_server, auth_cookies, "workflow_engine_api", {"action": "get_dashboard"}
    )
    assert "success" in data or "ok" in data, f"Expected 'success' or 'ok' key in response: {data}"
...
def test_workflow_dashboard(app_server, auth_cookies):
    ...
    assert "stats" in data or "dashboard" in data, (
        f"Expected 'stats' or 'dashboard' key in response: {data}"
    )
...
def test_workflow_get(app_server, auth_cookies):
    ...
    assert "runs" in data or "workflows" in data, (
        f"Expected 'runs' or 'workflows' key in response: {data}"
    )
...
def test_workflow_get_status_missing_id(app_server, auth_cookies):
    ...
    try:
        data = api_post(
            app_server, auth_cookies, "workflow_engine_api", {"action": "get_status"}
        )
