...
def test_memory_dashboard_get_subdirs(app_server, auth_cookies):
    ...
    data = api_post(app_server, auth_cookies, "memory_dashboard", {
    ...
    assert isinstance(data["subdirs"], list), (
        f"Expected 'subdirs' to be a list, got: {type(data['subdirs'])}"
    )
...
def test_memory_dashboard_search_default(app_server, auth_cookies):
    ...
    assert isinstance(data["memories"], list), (
        f"Expected 'memories' to be a list, got: {type(data['memories'])}"
    )
    assert "total_count" in data, f"Expected 'total_count' key: {data}"
    assert isinstance(data["total_count"], int), (
        f"Expected total_count to be int, got: {type(data['total_count'])}"
    )
