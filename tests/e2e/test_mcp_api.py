...
def test_mcp_status_returns_structure(app_server, auth_cookies):
    ...
    assert "status" in data or "servers" in data, (
        f"Expected 'status' or 'servers' key in response: {data}"
    )
...
def test_mcp_detail_unknown_server(app_server, auth_cookies):
    ...
    try:
        data = api_post(
            app_server, auth_cookies, "mcp_server_get_detail", {"server_name": "nonexistent_xyz"}
        )
...
def test_mcp_tools_reload(app_server, auth_cookies):
    ...
    assert "success" in data or "ok" in data, f"Expected 'success' or 'ok' key in response: {data}"
    assert "cache" in data or "tools" in data, (
        f"Expected 'cache' or 'tools' key in response: {data}"
    )
