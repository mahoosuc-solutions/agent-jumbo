...
def extract_json_string(content):
    ...
    pattern = (
        r'\{(?:[^{}]|(?R))*\}|\[(?:[^\[\]]|(?R))*\]|"(?:\\.|[^"\\])*"|true|false|null|-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?'
    )
...
def load_classes_from_folder(
    ...
    for file_name in py_files:
        ...
        # Keep server startup resilient: skip modules that fail to import.
        try:
            module = import_module(file_path)
        except Exception as e:
            if isinstance(e, ModuleNotFoundError) and str(e).startswith("No module named 'instruments.custom."):
                continue
            print(f"Skipping module {file_name}: {e}")
            continue
