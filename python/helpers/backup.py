...
"""  # nosec B108 - tmp/ is relative to agent root, not system /tmp
    ...
    async def test_patterns(self, metadata: dict[str, Any], max_files: int = 1000) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for _base_pattern_path, base_real_path in self.base_paths.items():
        ...
        try:
            with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zipf:
                ...
                metadata = {
                    ...
                    # File information
                    "files": [
                        {"path": f["path"], "size": f["size"], "modified": f["modified"], "type": "file"}
    ...
    async def preview_restore(
        ...
        try:
            ...
            with zipfile.ZipFile(temp_file, "r") as zipf:
                ...
                archive_files = [name for name in zipf.namelist() if name not in ["metadata.json", "checksums.json"]]
                ...
                for archive_path in archive_files:
                    ...
                    # so that patterns like "/home/rafael/aj/data/**" can match files correctly
                    ...
                    if os.path.exists(target_path) and overwrite_policy == "skip":
                        skipped_files.append(
                            {
                                "archive_path": archive_path,
                                "original_path": original_path,
            ...
            with zipfile.ZipFile(temp_file, "r") as zipf:
                ...
                for archive_path in archive_files:
                    ...
                    try:
                        ...
                        errors.append({"path": archive_path, "original_path": original_path, "error": str(e)})
