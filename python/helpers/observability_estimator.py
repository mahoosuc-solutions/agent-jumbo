...
def estimate_project(project: dict[str, Any], defaults: dict[str, Any], pricing: PricingProfile) -> dict[str, Any]:
    ...
    commercial_variable_usd = (billable_events / max(pricing.billing_unit, 1)) * pricing.unit_price_usd
...
def estimate_portfolio(payload: dict[str, Any]) -> dict[str, Any]:
    pricing_raw = payload.get("pricing_profile", {})
    pricing = PricingProfile(
        seat_price_usd=_float(pricing_raw.get("seat_price_usd", 39)),
        included_events=_float(pricing_raw.get("included_events", 10000)),
        billing_unit=_float(pricing_raw.get("billing_unit", 1000)),
        unit_price_usd=_float(pricing_raw.get("unit_price_usd", 0.5)),
    )
